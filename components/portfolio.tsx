...
function ProjectCard({ label, title, context, contribution, outcome, link }: { label: string, title: string, context: string, contribution: string, outcome: string, link?: string }) {
  return <article className="project-card"><p className="project-label">{label}</p><h3>{title}</h3><div className="project-story"><div><span>Context</span><p>{context}</p></div><div><span>My contribution</span><p>{contribution}</p></div><div className="outcome"><span>Outcome</span><p>{outcome}</p></div></div>{link && <a className="project-link" href={link} target="_blank" rel="noreferrer">View on GitHub <ArrowUpRight size={14}/></a>}</article>
}
